...
@cython.wraparound(False)
@cython.cdivision(True)
cdef inline void _sph_projection_deposit(
        np.float64_t posx_j, np.float64_t posy_j, np.float64_t posz_j,
        np.float64_t hsml_j, np.float64_t pmass_j, np.float64_t pdens_j,
        np.float64_t weight_j,
        np.float64_t * quantity, int ncomp,
        np.float64_t * local_buff,
        np.uint8_t[:, :] mask,
        np.float64_t x_min, np.float64_t x_max,
        np.float64_t y_min, np.float64_t y_max,
        np.float64_t z_min, np.float64_t z_max,
        np.float64_t dx, np.float64_t dy,
        np.float64_t idx, np.float64_t idy,
        np.intp_t xsize, np.intp_t ysize,
        np.float64_t period_x, np.float64_t period_y, np.float64_t period_z,
        np.int8_t * check_period,
        int * xiter, int * yiter, int * ziter,
        np.float64_t * xiterv, np.float64_t * yiterv, np.float64_t * ziterv,
        SPHKernelInterpolationTable itab,
        ) noexcept nogil:
    # Deposit a single particle's contribution onto the (component-major)
    # thread-local buffer.  This holds the geometry shared by the scalar and
    # vector SPH projection kernels: the expensive neighbor/pixel search is done
    # once and the per-pixel kernel weight is multiplied into every one of the
    # ncomp components.  For ncomp == 1 this reproduces the scalar projection
    # exactly.
    cdef np.int64_t xi, yi, x0, x1, y0, y1, pix
    cdef np.float64_t q_ij2, posx_diff, posy_diff, ih_j2, h_j2
    cdef np.float64_t x, y, sx, sy, sz, prefactor_base, contribution
    cdef int ii, jj, kk, m

    xiter[1] = yiter[1] = ziter[1] = 999

    if check_period[0] == 1:
        if posx_j - hsml_j < x_min:
            xiter[1] = +1
            xiterv[1] = period_x
        elif posx_j + hsml_j > x_max:
            xiter[1] = -1
            xiterv[1] = -period_x
    if check_period[1] == 1:
        if posy_j - hsml_j < y_min:
            yiter[1] = +1
            yiterv[1] = period_y
        elif posy_j + hsml_j > y_max:
            yiter[1] = -1
            yiterv[1] = -period_y
    if check_period[2] == 1:
        if posz_j - hsml_j < z_min:
            ziter[1] = +1
            ziterv[1] = period_z
        elif posz_j + hsml_j > z_max:
            ziter[1] = -1
            ziterv[1] = -period_z
    ...
    prefactor_base = pmass_j / pdens_j / hsml_j**2 * weight_j
    ...
    for kk in range(2):
        ...
        #if (sz + hsml_j < z_min) or (sz  - hsml_j > z_max): continue
        if (sz < z_min) or (sz > z_max): continue

        for ii in range(2):
            if xiter[ii] == 999: continue
            sx = posx_j + xiterv[ii]
            if (sx + hsml_j < x_min) or (sx - hsml_j > x_max): continue
            for jj in range(2):
                if yiter[jj] == 999: continue
                sy = posy_j + yiterv[jj]
                if (sy + hsml_j < y_min) or (sy - hsml_j > y_max): continue

                # here we find the pixels which this particle contributes to
                x0 = <np.int64_t> ((sx - hsml_j - x_min)*idx)
                x1 = <np.int64_t> ((sx + hsml_j - x_min)*idx)
                x0 = iclip(x0-1, 0, xsize)
                x1 = iclip(x1+1, 0, xsize)

                y0 = <np.int64_t> ((sy - hsml_j - y_min)*idy)
                y1 = <np.int64_t> ((sy + hsml_j - y_min)*idy)
                ...
                for xi in range(x0, x1):
                    ...
                    for yi in range(y0, y1):
                        ...
                        contribution = prefactor_base * itab.interpolate(q_ij2)
                        pix = (xi + yi*xsize) * ncomp
                        for m in range(ncomp):
                            local_buff[pix + m] += contribution * quantity[m]
                        mask[xi, yi] = 1
...
def pixelize_sph_kernel_projection(
    ...
    cdef np.int64_t xxi, yyi
    cdef np.float64_t dx, dy, idx, idy
    cdef np.float64_t period_x = 0, period_y = 0, period_z = 0
    cdef np.float64_t wj
    cdef int i, j
    cdef np.float64_t[:] _weight_field
    cdef np.float64_t * qbuf
    cdef int * xiter
    cdef int * yiter
    cdef int * ziter
    ...
    with nogil, parallel():
        ...
        qbuf = <np.float64_t *> malloc(sizeof(np.float64_t) * 1)
        ...
        for j in prange(0, posx.shape[0], schedule="dynamic"):
            ...
            qbuf[0] = quantity_to_smooth[j]
            wj = 1.0
            if weight_field is not None:
                wj = _weight_field[j]
            _sph_projection_deposit(
                posx[j], posy[j], posz[j], hsml[j], pmass[j], pdens[j],
                wj, qbuf, 1, local_buff, mask,
                x_min, x_max, y_min, y_max, z_min, z_max,
                dx, dy, idx, idy, xsize, ysize,
                period_x, period_y, period_z, &check_period[0],
                xiter, yiter, ziter, xiterv, yiterv, ziterv, itab)
...
@cython.wraparound(False)
@cython.cdivision(True)
def pixelize_sph_kernel_projection_vector(
        np.float64_t[:, :, :] buff,
        np.uint8_t[:, :] mask,
        any_float[:] posx,
        any_float[:] posy,
        ...
        any_float[:] pmass,
        any_float[:] pdens,
        np.float64_t[:, ::1] quantity_to_smooth,
        bounds,
        kernel_name="cubic",
        weight_field=None,
        _check_period = (1, 1, 1),
        period=None):
    # Vector-valued analogue of pixelize_sph_kernel_projection.  The quantity to
    # smooth is 2D, (nparticles, ncomponents), and the buffer is 3D,
    # (xsize, ysize, ncomponents).  All ncomponents are deposited in a single
    # pass over the particles -- the expensive neighbor search is shared via
    # _sph_projection_deposit, so this costs essentially the same as one scalar
    # projection rather than ncomponents of them.

    cdef np.intp_t xsize, ysize
    cdef int ncomp = quantity_to_smooth.shape[1]
    cdef np.float64_t x_min, x_max, y_min, y_max, z_min, z_max
    cdef np.int64_t xxi, yyi
    cdef np.float64_t dx, dy, idx, idy
    cdef np.float64_t period_x = 0, period_y = 0, period_z = 0
    ...
    cdef SPHKernelInterpolationTable itab = kernel_tables[kernel_name]
    with nogil, parallel():
        local_buff = <np.float64_t *> malloc(sizeof(np.float64_t) * xsize * ysize * ncomp)
        xiterv = <np.float64_t *> malloc(sizeof(np.float64_t) * 2)
        yiterv = <np.float64_t *> malloc(sizeof(np.float64_t) * 2)
        ziterv = <np.float64_t *> malloc(sizeof(np.float64_t) * 2)
        xiter = <int *> malloc(sizeof(int) * 2)
        ...
        xiter[0] = yiter[0] = ziter[0] = 0
        xiterv[0] = yiterv[0] = ziterv[0] = 0.0
        for i in range(xsize * ysize * ncomp):
            local_buff[i] = 0.0
        ...
        for j in prange(0, posx.shape[0], schedule="dynamic"):
            ...
            _sph_projection_deposit(
                posx[j], posy[j], posz[j], hsml[j], pmass[j], pdens[j],
                wj, &quantity_to_smooth[j, 0], ncomp, local_buff, mask,
                x_min, x_max, y_min, y_max, z_min, z_max,
                dx, dy, idx, idy, xsize, ysize,
                period_x, period_y, period_z, &check_period[0],
                xiter, yiter, ziter, xiterv, yiterv, ziterv, itab)
        ...
        with gil:
            for xxi in range(xsize):
                for yyi in range(ysize):
                    for m in range(ncomp):
                        buff[xxi, yyi, m] += local_buff[(xxi + yyi*xsize)*ncomp + m]
        free(local_buff)
        free(xiterv)
        free(yiterv)
        free(ziterv)
